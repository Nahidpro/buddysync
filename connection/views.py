...
from connection.models import Follow, Friendship
from django.contrib.auth import get_user_model
from connection.serializers import FollowSerializer
...
class FollowAPIView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response({"error": "User to follow does not exist"}, status=status.HTTP_400_BAD_REQUEST)
...
class UnfollowAPIView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response({"error": "User to unfollow does not exist"}, status=status.HTTP_400_BAD_REQUEST)

     
        if followed_user.id == request.user.id:
            return Response({"error": "You cannot unfollow yourself"}, status=status.HTTP_400_BAD_REQUEST)

        # Check if the user is actually following this user
        try:
...
class FollowingListAPIView(ListAPIView):
    ...
    def get_queryset(self):
        return Follow.objects.filter(following_user=self.request.user)
